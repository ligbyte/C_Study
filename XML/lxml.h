...
int ends_with(const WCHAR* haystack, const WCHAR* needle)
{
...
XMLNodeList* XMLNode_children(XMLNode* parent, const WCHAR* tag);
WCHAR* XMLNode_attr_val(XMLNode* node, WCHAR* key);
XMLAttribute* XMLNode_attr(XMLNode* node, WCHAR* key);
...
int XMLDocument_load(XMLDocument* doc, const WCHAR* path);
int XMLDocument_write(XMLDocument* doc, const WCHAR* path, int indent);
...
WCHAR* XMLNode_attr_val(XMLNode* node, char* key)
{
    for (int i = 0; i < node->attributes.size; i++) {
...
static TagType parse_attrs(WCHAR* buf, int* i, WCHAR* lex, int* lexi, XMLNode* curr_node)
{
...
int XMLDocument_load(XMLDocument* doc, const WCHAR* path)
{
    ...
    WCHAR* buf = (WCHAR*) malloc(sizeof(char) * size + 1);
